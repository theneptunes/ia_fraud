...
def train(df):
    ...
    y_pred = model.predict(X_test)
    print('Gaussion Naive Bayes Done')
...
def get_model():
    return torch.load('models/naive.pt')

def save(model):
    torch.save(model,'models/naive.pt')

def predict_nb(t: Transaction.Transaction):
    model = get_model()
...
if __name__ == "__main__":
    ...
    sys.modules[__name__] = predict_nb
